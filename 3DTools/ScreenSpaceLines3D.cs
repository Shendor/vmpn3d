...
namespace _3DTools
{
    ...
    public class ScreenSpaceLines3D : ModelVisual3D
    {
        public ScreenSpaceLines3D()
        {
            ...
            unlitMaterial = new MaterialGroup();
        ...
        public static readonly DependencyProperty TextureSource =
           DependencyProperty.Register(
               "TextureSource",
               typeof(Uri),
               typeof(ScreenSpaceLines3D),
               new PropertyMetadata(
                   new Uri("pack://application:,,,/Materials;component/Content/ArcTextures/ArcDirect.jpg"),
                   OnTextureChanged));

        private static void OnTextureChanged(DependencyObject sender, DependencyPropertyChangedEventArgs args)
        {
            ((ScreenSpaceLines3D)sender).SetTexture((Uri)args.NewValue);
        }

        public void SetTexture(Uri source)
        {
            ImageSource img = new BitmapImage(source);
            ...
            unlitMaterial.Children.Add(new EmissiveMaterial(iBrush));
            unlitMaterial.Freeze();
            geometry.Material = unlitMaterial;
            geometry.BackMaterial = unlitMaterial;
        }

        public Uri Texture
        {
            get { return (Uri)GetValue(TextureSource); }
            set { SetValue(TextureSource, value);}
        }
        ...
        {
            get { return unlitMaterial; }
            set { unlitMaterial = value; }
        }
        ...
        {
            if (_mesh.Positions == null || Points.Count == 0 && _mesh.Positions.Count == 0)
            {
                return;
            }
        ...
        protected MeshGeometry3D _mesh;
        protected MaterialGroup unlitMaterial;
        protected Uri textureSource;
    }
}
